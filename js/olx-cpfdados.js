...
// 🔑 COLE SEU COOKIE COMPLETO AQUI
const COOKIE_STRING = `COLE_AQUI_SEU_COOKIE`;

// 🔄 Converter cookie string → objeto
...
// 🚀 NOVA FUNÇÃO PRINCIPAL (substitui axios)
async function buscarInfoComId(listId) {
  let browser;
  ...
  try {
    ...
    if (dadosVendedor) {
      ...
      console.log("✅ Resultado:", resultDadosOlx);

      await browser.close();
      return resultDadosOlx;
    } else {
      console.log(`❌ Não conseguiu extrair ${listId}`);
      await browser.close();
      return null;
    }
  } catch (error) {
